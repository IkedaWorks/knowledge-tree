...
| **`fix`**      | Correção de erros em fórmulas LaTeX, links quebrados ou bugs de script.     | `fix(latex): correct Maxwell equation`   |
| **`style`**    | Formatação e ajustes visuais (Markdown, espaçamento) sem alterar o sentido. | `style(md): fix indentation in intro`    |
| **`chore`**    | Manutenção de rotina (atualização de `.gitignore`, licenças ou metadados).  | `chore: update license year`             |
