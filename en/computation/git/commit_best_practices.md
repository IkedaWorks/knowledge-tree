...
| **`fix`** | 🛠️ Correcting errors in LaTeX formulas, broken links, or script bugs. | `fix(latex): correct Maxwell equation` |
| **`style`** | 🎨 Formatting and visual adjustments (Markdown, spacing) without changing meaning. | `style(md): fix indentation in intro` |
| **`chore`** | ⚙️ Routine maintenance (updating `.gitignore`, licenses, or metadata). | `chore: update license year` |
